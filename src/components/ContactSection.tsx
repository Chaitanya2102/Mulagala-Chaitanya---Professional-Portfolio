...
import { Mail, Phone, MapPin, Send, CheckCircle2, Copy, Check, MessageSquare, Building, Download, ArrowUpRight, AlertCircle, Loader2, RefreshCw } from 'lucide-react';
import { motion } from 'motion/react';

const WEB3FORMS_ACCESS_KEY = import.meta.env.VITE_WEB3FORMS_ACCESS_KEY || '919e0b83-37f4-4c9e-9486-7d283cb640a4';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        subject: `[Portfolio] ${formData.subject} (${formData.inquiryType})`,
      ...
      if (response.ok && data.success) {
        ...
        setErrorMessage(data.message || 'Submission failed. Please try again or reach out directly via email.');
      }
    } catch (err: any) {
      console.error('Web3Forms Error:', err);
      setErrorMessage('Network connection error. Please use the "Open in Mail Client" button below or email directly.');
    } finally {
            ...
            {!isSubmitted ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-200">
                  ...
                  <span className="text-[10px] sm:text-[11px] font-mono text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 font-semibold flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Direct Delivery
                  </span>
                </div>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-rose-600 mt-0.5">
                        ...
                        <a href="mailto:mulagalachaitanya@gmail.com" className="underline font-mono">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="Casual Chat / Catch-up">Casual Chat / Catch-up</option>
              ...
              >
                ...
                <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
                  Thank you, <strong className="text-slate-900">{formData.name}</strong>. Your message regarding <strong className="text-blue-600">"{formData.subject}"</strong> has been safely delivered to Mulagala Chaitanya's inbox.
                </p>
                <p className="text-xs text-slate-500 font-mono">
                  A response will be sent to <span className="text-blue-600 font-semibold">{formData.email}</span> within 24 hours.
                </p>
                ...
                <div className="pt-4 flex flex-wrap justify-center gap-3">
                  ...
                  >
                    <Mail className="w-3.5 h-3.5" />
                    <span>Also open in Gmail / Outlook</span>
                  </a>
                  <button
                    onClick={() => {
                    ...
                    className="px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 text-xs font-semibold cursor-pointer transition shadow-2xs"
                  >
