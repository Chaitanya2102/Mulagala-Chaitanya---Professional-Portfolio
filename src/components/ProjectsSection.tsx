...
export const ProjectsSection: React.FC = () => {
  ...
  return (
    <section id="works" className="relative py-20 md:py-28 bg-slate-50/80 border-t border-slate-200/60 overflow-hidden">
          ...
          <AnimatePresence>
            {filteredProjects.map((project, idx) => (
              <motion.div
                ...
                className="group bg-white border border-slate-200/80 rounded-3xl overflow-hidden hover:border-blue-400 hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="relative aspect-[16/9] bg-slate-100 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
                      <span className="px-3 py-1 rounded-full text-xs font-semibold bg-white/95 backdrop-blur-md text-blue-700 shadow-sm">
                        {project.categoryLabel}
                      </span>
                      <span className="px-2.5 py-1 rounded-lg text-[11px] font-mono bg-white/95 backdrop-blur-md text-slate-700 shadow-sm">
                        {project.clientOrContext}
                      </span>
                    </div>

                    {/* Hover Quick Prompt Overlay */}
                    <div className="absolute inset-0 bg-blue-600/85 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition duration-300 flex items-center justify-center">
                      <span className="px-5 py-2.5 rounded-xl bg-white text-blue-700 font-bold text-xs flex items-center gap-2 shadow-lg">
                        <Sparkles className="w-4 h-4 text-blue-600" />
                        Explore Interactive Demo & Architecture
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-7 space-y-4">
                    <div>
                      <h3 className="text-xl font-bold text-slate-900 group-hover:text-blue-600 transition flex items-center justify-between">
                        ...
                        <ArrowUpRight className="w-4 h-4 text-slate-400 group-hover:text-blue-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition" />
                      </h3>
                    ...
                    {/* Metrics preview */}
                    {project.metrics && (
                      <div className="grid grid-cols-2 gap-2.5 pt-2 border-t border-slate-100 font-mono text-center">
                ...
                {/* Tech Stack Footer */}
                <div className="px-7 py-4 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap gap-1.5">
                    {project.techStack.slice(0, 4).map(tech => (
                      <span
                        key={tech}
                        className="px-2.5 py-0.5 rounded-lg text-[11px] font-mono bg-white border border-slate-200 text-slate-700 font-medium"
                      >
                        {tech}
                      </span>
                    ))}
