...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="relative py-20 md:py-28 bg-white/80 border-t border-slate-200/60 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        >
          ...
          <div className="max-w-md mx-auto mt-6 relative">
            ...
            <input
              type="text"
              placeholder="Search skills (e.g. COBOL, DB2, Python, VSAM, SQL)..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-slate-900 focus:outline-none focus:border-blue-600 focus:bg-white placeholder:text-slate-400 transition"
            />
            {searchQuery && (
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSkills.map(skill => (
                <motion.div
                  ...
                  className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition space-y-2.5"
                >
              ...
              {skillCategories.map(cat => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div className="text-xs font-bold truncate text-slate-900">{cat.title}</div>
                      <div className="text-[11px] font-mono text-slate-500">{cat.skills.length} competencies</div>
                    </div>
                  </motion.button>
                );
              })}
              ...
              >
                {currentCategory.skills.map((skill, idx) => (
                  <motion.div
                    ...
                    className="bg-white p-5 rounded-2xl border border-slate-200 hover:shadow-md transition space-y-3 flex flex-col justify-between"
                  >
                    <div>
